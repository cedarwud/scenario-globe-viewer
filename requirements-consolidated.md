...
- 本文件不取代 `itri-acceptance-report-2026-04-20/` 的驗收狀態追蹤；後者仍是驗收進度的權威。
...
- **B** = 需要 ITRI 提供 operator data 才能達 Tier A 權威的項目;本專案已實作 Tier B 公開來源替代方案。
- **C** = ITRI 既有 infra / 報告層交付,**非本專案 UI scope** — 列出僅為需求清單完整性。
...
| ID | 來源 | 需求 | 完成度 |
|---|---|---|---|
| R1-T1 / K-A1 | r1 + slide 2 | 整合 ITRI 軌道模型 LEO/MEO/GEO + 多軌道訊號切換 | done — 視覺化完成;多軌道切換邏輯已接線 (link-budget → runtime-projection，2026-05-17)。連續 LIVE 跨軌切換之細化見 V-MO1 |
...
### B. 需 ITRI 提供 operator data 才能達 Tier A 權威 (Tier B 替代已實作)

本桶為**本專案的 Tier B 替代工作** — 每條已於本專案以公開標準自寫實作;ITRI 交付 operator data 時 swap B → A (Tier A)。

| ID | 來源 | 內容 | 替代方案 (Tier B) |
|---|---|---|---|
| K-A2 | slide 2 | 鏈路品質規則 (latency / jitter / network speed 切換 policy) | 3GPP TR 38.811 §6.7 + TR 38.821 §7.3 self-implement — Tier B 替代**已實作** (`runtime-projection.ts` latency + `handover-policy.ts`，2026-05-17)；待 ITRI operator data swap → Tier A |
| K-A3-a | slide 2 | 天線參數 (peak gain、beamwidth、pattern) | ITU-R S.1528 (LEO/non-GSO) + S.465 (Earth station) self-implement — Tier B 替代**已實作為 standalone module** `src/runtime/link-budget/antenna-pattern.ts` (2026-05-17,exports `computeSatelliteAntennaGainDb` + `computeEarthStationAntennaGainDb`)；目前 V4 runtime-projection 視覺路徑**尚未**接線 (only FSPL + gas + rain de-rate downlink throughput);bootstrap-physical-input-seeds 仍用既有 `itu-r-f699-antenna-pattern.ts`。新模組可由直接呼叫或未來接線 demo;待 ITRI operator data swap → Tier A |
| K-A3-b | slide 2 | 雨天衰減模型 | ITU-R P.618-14 §2.2.1 + P.676-13 self-implement — Tier B 替代**已實作** (`rain-attenuation.ts` + `gas-absorption.ts`，2026-05-17)；待 ITRI operator data swap → Tier A |
| K-F2 | slide 6 | 整合 V 組模擬程式 (天線 + 雨衰 + ITU 規範) | 自寫 ITU calculator wrapping P.618 / P.676 / S.1528 / S.465 — Tier B 替代**已實作** (`src/runtime/link-budget/` 五模組，2026-05-17:FSPL + 雨衰 + 氣衰 + handover-policy 已接線進 runtime-projection,antenna-pattern 為 standalone 尚未接線進視覺路徑);待 ITRI V 組程式 swap → Tier A |
| (irreducible-1) | — | ITRI ESTNeT packet trace (真實 latency / jitter time series) | synthetic baseline from TR 38.811 §6.7 generic model；truth-boundary tag `"ITRI ESTNeT trace pending"` |
| (irreducible-2) | — | ITRI 驗收測試 scenario 腳本 (哪個 case 該 pass、閾值多少) | 5 候選 demo URL 已列於 `scenario-globe-viewer/docs/itri-requirement-walkthrough.md` §3 (high/low elevation × rain/clear × handover/no-handover 代表性子集,2026-05-17);閾值仍待 ITRI acceptance script swap |
...
### C. ITRI 既有 infra / 報告層 (非本專案 UI scope)
...
| K-A5 | slide 2 | Linux 主環境 + Windows + WSL 備案 | ITRI 既有 infra |
| K-B1 | slide 3 | Windows Tunneling Scenario / ESTNeT Real-Traffic Satellite Bridge (`ping 10.2.0.1 → tun0 → GS-A → Satellite → GS-B → tun1`) | ITRI 既有拓樸示意，非 UI 規格 |
| K-C1 | slide 4 | INET NAT routing + veth0 (192.168.2.1) / veth1 (192.168.2.2) + 模擬-真實網路橋接 | ITRI 既有 |
| K-F5 | slide 6 | 虛擬待測物 — 撰寫 testbench 測試程式 | ITRI scope (非 UI) |
| K-F6 | slide 6 | 實體待測物 — NE-ONE Traffic Generator 情境模擬 | ITRI scope (非 UI) |
| K-F8 | slide 6 | 期末報告撰寫 | 本專案撰寫 (報告層，非 UI 模組)；與 R1-D5 部分重疊但屬於不同階段 |
| R1-D5 | r1 交付表 | 技術 WP1 評估分析報告一式 | 本專案撰寫 (報告層) |

C 合計：**8 條** (其中 R1-D5 + K-F8 為本專案文書層交付，其他 6 條為 ITRI infra)。
...
| B 需 ITRI 資料 / Tier B 替代已實作 | 7 | 含 3 條 irreducible substitute |
| C ITRI 已完成 / infra | 8 | 含 R1-D5 + K-F8 報告層 |
| **合計** | **34** | |

**本專案主軸 = Bucket A 共 19 條** (18 文件 + 1 口頭)。Bucket B 為本專案 Tier B 替代工作 (依賴 ITRI operator data 升等 → Tier A);Bucket C 屬 ITRI scope。

## Source authority tier 註解

- **Tier A operator-validated**：ITRI 自家 retained data；目前 0 條已交付，Bucket B 全部 7 條 (K-A2 / K-A3-a / K-A3-b / K-F2 + 3 條 irreducible) 待 V 組 / 驗收工程師後續 swap。
...
- **2026-05-17**：從 `r1.docx` + `kickoff.pptx` 去重整理初版；加入 V-MO1 (口頭追加跨軌道 live handover)；加入 Bucket B/C 分類；標註 ITRI infra 已完成項目與 irreducibly-only 項目。
...
- **2026-05-17 (B/C relabel)**：重排桶順序為 demo 相關性遞減 — 新 **B = 舊 C** (7 條,需 ITRI 資料但 Tier B 替代已實作,屬本專案交付);新 **C = 舊 B** (8 條,ITRI 既有 infra / 報告層,非本專案 UI scope)。A 不變,所有 ID 與計數本質未變,只字母互換 + 排序更動以反映 demo 優先序 (本專案交付物在前,ITRI 既有 infra 在後)。
...
- **2026-05-17 (short URL + irreducible-2)**：V4 demo route 縮短為 `/?stationA=<id>&stationB=<id>` (合法 station id 即自動啟用 V4 場景與 regional 預設;長形 URL 仍支援)。Bucket B irreducible-2 替代清單以 5 個短 URL 形式落入 `scenario-globe-viewer/docs/itri-requirement-walkthrough.md` §3 (對應 high/low elevation × rain/clear × handover/no-handover 代表性子集)。
...
- [itri-acceptance-report-2026-04-20/](itri-acceptance-report-2026-04-20/) — 驗收狀態追蹤
