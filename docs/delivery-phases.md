...
- Phase 5.1 now adds the first live satellite runtime path under a constrained boundary: a repo-owned top-level overlay controller stays off by default, loads the copied walker fixture only when enabled, renders point-only entities through runtime-specific modules under `src/runtime/`, and detaches that overlay path cleanly when disabled again.
- The current Phase 5.1 slice does not add labels, orbits, per-satellite controls, overlay HUD controls, or richer scenario data. Native toolbar, timeline, credits, and the status-only HUD remain the accepted shell.
...
- enabling the slice loads the copied walker TLE, reuses the existing walker ingestion seam, and renders point-only entities through runtime-specific Cesium data-source modules without moving parsing, propagation, or frame conversion into `overlay-manager`
- disabling the slice detaches the point overlay cleanly and restores the accepted globe-only validation path without adding overlay HUD controls
- labels, orbit polylines, per-satellite controls, and richer scenario data remain out of scope
