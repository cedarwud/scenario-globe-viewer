...
- Phase 7.0 soak evidence now has a repo-owned boundary in `docs/data-contracts/soak-evidence.md` plus a repeatable harness at `tests/soak/run-soak.mjs`; rehearsal artifacts exist under `output/soak/` and stay out of git; the retained `24h` full-run pass artifact has not yet been produced and remains pending.
