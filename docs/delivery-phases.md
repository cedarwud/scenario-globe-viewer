...
- Phases 5.1-5.2 now add the first live satellite runtime path under a constrained boundary: a repo-owned top-level overlay controller stays off by default, loads the copied walker fixture only when enabled, renders the landed walker point entities plus fixed runtime-local labels from existing `name` or fallback `id` through runtime-specific modules under `src/runtime/`, and detaches that overlay path cleanly when disabled again.
- The current Phase 5.2 slice widens only into those fixed labels on the existing point path. Orbits, per-satellite controls, overlay HUD controls, and richer scenario data remain out of scope. Native toolbar, timeline, credits, and the status-only HUD remain the accepted shell.
...
- enabling the slice loads the copied walker TLE, reuses the existing walker ingestion seam, and renders the landed point entities plus fixed runtime-local labels derived only from existing `name` or fallback `id` through runtime-specific Cesium data-source modules without moving parsing, propagation, or frame conversion into `overlay-manager`
- disabling the slice detaches the point-and-label overlay cleanly and restores the accepted globe-only validation path without adding overlay HUD controls
- orbit polylines, per-satellite controls, richer scenario data, and user-facing label styling/UI remain out of scope
