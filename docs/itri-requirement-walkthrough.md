...
- **完成度**: done,超過門檻。demo 載入的 LEO actor fixture 為 600 顆;
  上游未裁切來源清單約 11015 顆 (Starlink + OneWeb,2026-05-16)。
- **demo 驗證位置**: LEO actor count chip (`chrome.topLeft` 第三列右側,
  `data-leo-actor-count=600`)。
- **source tier**: Tier C — TLE-derived。
- **重現**: chrome chip 讀取 demo LEO actor fixture 計數;runtime-projection
  為控制效能每軌道上限取 60 顆參與配對運算 (此為運算上限,非 actor fixture
  總數上限)。
- **工程師可能問**: 「為何 panel 只算 60 顆?」→ 60 顆是「配對可見性運算」的效能上限
  讓 panel 1 秒內完成;chrome chip 計數仍為 demo actor fixture 的 600 顆。
...
- [x] R1-F1 / K-E1 — ≥500 LEO:demo actor fixture 600 顆;上游來源約 11015 顆
...
4. **Q: ≥500 LEO 達標,但 panel 只算 60 顆?**
   A: 60 為「配對可見性運算」每軌道效能上限,確保 panel 1 秒內回應。chrome
   chip 顯示 demo actor fixture 的 600 顆;上游未裁切來源清單約 11015 顆。
   三者是不同的數量。
