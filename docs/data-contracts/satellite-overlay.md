...
`satellite-overlay` fixes the repo-facing serializable boundary for satellite fixture ingestion and overlay runtime work without widening the public contract whenever the live runtime changes. In the current repo reality, Phase 3 defines the public data shapes, the adapter interface, and the ownership split between `overlay-manager` and a later concrete satellite adapter. Phase 4 records the smallest allowed fixture-ingestion seam, and Phases 5.1-5.3 now layer a minimal walker point path plus fixed runtime-local labels and bounded orbit polylines on top of that seam while keeping the public contract itself stable.
...
The public contract fixes the outer data boundary even now that the repo has a minimal Phase 5.1-5.3 runtime path. Any conversion from plain-data fixture/sample input into Cesium runtime types belongs inside a concrete adapter implementation, not in the public contract and not in `overlay-manager`.
...
- Phases 5.1-5.3 now add runtime-specific modules under `src/runtime/`, including `runtime-overlay-manager.ts`, `walker-point-overlay-adapter.ts`, and `satellite-overlay-controller.ts`
...
- Phases 5.1-5.3 now consume that seam through runtime-specific point-backed modules under `src/runtime/`
- the default startup path keeps the overlay disabled, and the first live toggle path is limited to the repo-owned walker-backed point path plus fixed runtime-local labels and bounded Cesium polyline orbits only
- Phase 5.3 widens only into bounded runtime-local orbit polylines on top of the existing point-and-label path; `Entity.path`, per-satellite controls, overlay HUD controls, public label styling/configuration, and richer scenario data remain out of scope
...
- Phase 5.1 is the first landed slice that introduces overlay runtime/rendering work on top of the Phase 4 ingestion seam, Phase 5.2 widens that same path into fixed labels derived from existing `name` or fallback `id`, and Phase 5.3 widens only into bounded Cesium polyline orbits while keeping the single top-level enable/disable control surface

## Not-Yet-Implemented

This contract does not currently include:

- `Entity.path` or other unbounded live history rendering on the satellite path
- per-satellite visibility control
- HUD controls or user-facing overlay UI
- richer scenario data, public orbit or label styling/configuration, or additional fixture classifications beyond the current walker-backed point/label/polyline path
