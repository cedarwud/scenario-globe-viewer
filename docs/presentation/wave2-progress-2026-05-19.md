...
Deck 結構：前半 (slide 2–13) 回答審查上常問到的站點圓點意義、篩選條件、
命名、selected-pair 衛星與連線解讀、CSV 稽核、Rain rate 控制；後半
(slide 14–18) 整理「公開軌道資料模擬」(TLE-first) 目前已落地、仍在開發、
以及不可對外宣稱的邊界。最後三頁是外部來源索引；附錄列工程驗證依據，
不是外部引用來源。所有數字與來源性 claim 都要能追到外部資料、SDD section、
spike report 或本地 code path。
...
結論：圓點不是全部一樣；底層分成兩個能力等級，畫面再依「目前哪一個等級
數量較少」加上 pink outline 強調。站點支援的軌道與頻段來自公開來源索引
[1][6]-[10]；能切換 2 軌或 3 軌則由系統依站點欄位與幾何規則推導。

| 能力等級 | 目前站數 | 一般圓點 | Selected | 意義 |
| --- | --- | --- | --- | --- |
| 三軌能力（tri-capable） | 22 | 綠色，radius 5.5 px；目前為少數群組，加 pink outline | 綠色，radius 8 px，yellow outline | 至少能與某個 pair 共享 3 種可切換軌道類別 |
| 雙軌能力（dual-only） | 47 | 藍色，radius 4.5 px，dark outline | 藍色，radius 6.5 px，yellow outline | 可形成相容 pair，最多共享 2 種可切換軌道類別 |

少數群組規則：tri 與 dual 之中目前站數較少的群組會多套 pink outline。目前
tri (22) 少於 dual (47)，所以 22 個三軌站點都套上 pink outline；若未來
registry 變動造成 dual 反而較少，pink outline 會改套到 dual 群組。資料庫
目前沒有完全配不到 pair 的站，所以畫面上不會出現第三種樣式。

判斷方式：先要求 pair 有共同 RF 頻段，再檢查共同 LEO / MEO / GEO 是否通過
10° 以上仰角的幾何可視條件。某站可形成的 pair 最大共享 3 種軌道就是
tri-capable；最大為 2 種就是 dual-only。圓點顏色與大小只表示 handover
capability 與 hover / selection 狀態；資料來源等級仍放在畫面標籤、站點
資訊卡、以及 selected-pair 右側面板。
...
真實性邊界：每個站點都有一個 primary public source；只收公開來源可佐證
至少支援 2 種軌道類別 (LEO / MEO / GEO 任兩種) 的站。座標精度依
`disclosurePrecision` 欄位區分：可能是站點實際座標 (exact)，也可能是
鄉鎮層級或營運商家族區域代表座標 (representative)。Supported orbits /
bands 只代表營運商在公開資料中揭露的能力，不是實際服務證明。
`elevationM` / `terrainMaskDeg` 已寫入資料庫供之後 runtime 接讀，目前
還不顯示在 info card 上。
