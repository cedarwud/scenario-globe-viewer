...
站點支援的軌道與頻段來自公開來源索引 [1][7]-[11]；2-orbit / 3-orbit
...
來自公開站點資料庫 [1][7]-[11]；Handover 是系統用「共同頻段 + 共同軌道
幾何可視條件」推導出的 pair 篩選。
...
| Compute cap | 互動運算目前每個軌道類別最多取 60 筆 TLE records [6] |
| Geometry filter | 用 SGP4 propagation + station geometry 計算可視窗；門檻為 10° 仰角 |
| Cadence | 可視窗取樣間隔：LEO 30 s、MEO 60 s、GEO 120 s [6] |
...
snapshots [2]；station endpoint 來自公開站點資料庫 [1][7]-[11]。
...
| 類別 | 可以怎麼說 |
| --- | --- |
| 真實公開資料 | station identity / coordinates precision / public source、TLE satellite identity、NORAD / COSPAR、SATCAT summary [1][2][7]-[11] |
...
或 commercial routing evidence [6]。
...
模型輸入，可在不改站點與 TLE source truth 的前提下展示 weather fade stress test [4][6]。
...
來源：SDD §7、§11、§12.1；slice-0 §6.2；效能測試報告（內部編號 S1）[6]。

TLE-first 的意思：衛星身份與軌道先使用公開 TLE / SATCAT 資料，再用模型推導
站點可視窗、連線選擇與天候影響。它不是營運商實測資料，也不是真實封包紀錄 [2][12]。
...
| 已完成 | 對報告的意義 | 追溯依據 |
| --- | --- |
| 公開 TLE / SATCAT source path | 衛星身份與軌道來源可追溯，不補 fake actor | [2][6][12] |
| 模擬結果來源欄位 | CSV / 右側面板可追到資料來源、模型、non-claim | [6] |
| Station elevation prep | 69 個站點已有高度與 horizon mask 欄位，等待 runtime 接線 | [1][5][6] |
| Slice-0 baseline | 五個 walkthrough URL 已固定為後續 comparison 的基準 | [6] |
| Performance spike | LEO 200 cap at 30 s 已有 p95 391.2 ms evidence | [6] |
...
| 大氣 / 雨量格點 | cloud、scintillation、rain height、rain climatology grids | 需先確認 ITU-R grids 的使用 / 打包政策 [4][12] |

---

# 目前可驗證證據

來源：slice-0 §6.2 values 與 2026-05-19 重新擷取的 screenshots [6]。
...
| [6] | 工程追溯：本 deck 內列出的 SDD sections、slice-0 baseline、效能測試報告、本地程式碼路徑。 |
