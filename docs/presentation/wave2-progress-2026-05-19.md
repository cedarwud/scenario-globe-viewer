...
| TLE pool | 使用 CelesTrak public GP TLE snapshots；SATCAT summary 補 constellation / operator context |
...
CelesTrak snapshots；station endpoint 來自 public registry。
...
畫面上顯示哪幾顆：scene actor set 來自 pair visibility windows、handover
...
| Latency | 用 slant range 推估 one-way propagation delay；依據 3GPP TR 38.811 §6.7 |
...
P.676 / P.618；handover policy 參考 TR 38.821 §7.3 的 trigger metrics。
