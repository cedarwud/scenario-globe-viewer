# ITRI Unclosed Requirement Evidence Intake Matrix
...
- Intake type:
  first-round public-source evidence triage for unclosed ITRI requirements.
- Source rule:
...
does not close the full ITRI original requirement set.
...
- F-13 now has a route-exposed fixture/model-backed scale readiness surface
  ...
  complete, route-native scale closure is not claimed, and ITRI authority
...
| Group | Current gap | Required evidence | Owner / input needed | Public-source availability | ITRI-input questions | Testbed requirements | Allowed local prototype | Forbidden claims | Recommended next prompt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F-01 ITRI orbit model integration | Route uses viewer-owned projected artifacts and source-lineaged actors, not ITRI's self-developed orbit model. | ITRI orbit-model API/spec, input/output schema, time basis, reference frames, TLE or ephemeris ingestion rules, validation vectors, and acceptance criteria. | ITRI model owner. Requires private or explicitly released ITRI material. | ITRI public NTN/LEO pages provide NTN context only. No verified public ITRI self-developed orbit-model implementation spec was found. | What is the authoritative orbit model package? What frame and propagator are required? Which scenarios and vectors are acceptance truth? Can the model be redistributed or wrapped? | Deterministic model replay harness with known vectors and route-independent comparison output. | A read-only adapter scaffold using placeholder interfaces and no synthetic orbital truth. | Do not claim ITRI orbit-model integration, active satellite truth, or native orbit closure from public context pages. | Draft a single F-01 integration SDD amendment that lists required ITRI model files, adapter contract, validation vectors, and stop rules. Do not implement until ITRI supplies authority artifacts. |
| F-07/F-08 communication time and statistics measured truth | Current route and repo seams are bounded proxies or modeled windows. They do not retain measured `ping` or `iperf` communication-time transcripts. | Timestamped traffic test runs, topology, endpoints, packet path, command transcripts, raw output, parsed statistics, clock source, and redaction notes. | External validation owner plus ITRI testbed owner. | INET PingApp and INET throughput examples support simulation-side method ideas. ESnet iPerf supports throughput measurement tooling. They do not provide project-specific measured truth. | Which endpoints count as communication-time truth? Is `ping` RTT, availability window, packet loss, `iperf` interval, or all of these required? What sample size and pass/fail threshold apply? | Windows/WSL or Linux testbed with ESTNeT/INET path, route/interface inventory, NAT/tunnel state if used, and retained traffic transcripts. | Parser/normalizer for retained `ping` and `iperf3` logs, plus report schema draft. | Do not invent ping values, iperf values, jitter, packet loss, or availability percentages. Do not convert route modeled windows into measured truth. | Create one external-measurement evidence plan for F-07/F-08/F-09 together, with required commands, artifact layout, parser rules, and acceptance thresholds. |
...
- Standards authority for ITU-R rain, atmospheric, and antenna-related model
  families.
- ITRI public NTN/LEO context.

Public official sources are not enough for these closure items:

- ITRI's private orbit model implementation.
- ITRI V-group input schema and physical-layer parameters.
...
- F-13 fixture/model-backed readiness as route-native `>=500 LEO`
  closure/proof or ITRI authority acceptance;
