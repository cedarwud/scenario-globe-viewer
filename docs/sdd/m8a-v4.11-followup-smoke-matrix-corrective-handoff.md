...
| V4.9 | Correction A §Footer chip system intentionally makes ambient disclosure chips default-visible: `[Simulation view]`, `[TLE: CelesTrak NORAD GP · 2026-04-26]`, `[operator-family precision]`, `[13 actors]`, plus the top scope/replay context. These strings are removed from the V4.9 default-visible denied-list path only as Correction A ambient disclosure. Forbidden claims, metadata demotion, and transition non-blocking checks remain hard. |
