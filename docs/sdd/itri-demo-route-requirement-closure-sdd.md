...
| --- | --- | --- | --- | --- |
| F-01 | Integrate customer's own satellite orbit model | Route uses viewer-owned projected artifacts and source-lineaged actors, not an customer orbit-model integration. | `external-validation-required` | Do not claim closure. Keep source lineage visible. |
| F-02 | Multi-orbit coverage of `LEO/MEO/GEO` | Route shows LEO/MEO/GEO display-context actors and operator-family evidence. | `bounded-route-representation` | Preserve 13-actor orbit-class evidence and compact truth labels. |
| F-03 | Ingest `TLE` and track satellite motion | Route uses source-lineaged display actors; it is not a general TLE input workflow. | `bounded-route-representation` | Keep evidence/archive source summary, but do not claim full TLE input closure. |
| F-04 | Build an interactive 3D graphical simulation system | Cesium route is interactive and visual. | `true-route-closure` for visual route baseline | Regression guard only. |
| F-05 | Provide a UI control surface | Route has rail, inspector, footer, truth surfaces, and replay controls. | `true-route-closure` for this route | Regression guard only. |
| F-06 | UI with dynamically adjustable parameters | Route has bounded replay and review controls, not a full rule-parameter editor. | `bounded-route-representation` | Do not let this substitute for F-11. |
| F-07 | Display communication time availability | Existing repo seam is mostly elsewhere; this route has modeled windows/countdown, not measured communication time. | `bounded-repo-owned-seam` | Show only route-local modeled timing; link to gap/status if needed. |
| F-08 | Produce communication-time statistics | Export-ready bounded report state exists elsewhere; route does not expose a completed communication-time statistics workflow. | `bounded-repo-owned-seam` | Keep as repo seam; the F-16 bounded route export does not close communication-time statistics. |
| F-09 | Display communication rate | Route exposes a bounded modeled network-speed class/proxy surface. It is not measured throughput. | `bounded-route-representation`; external throughput truth remains `external-validation-required` | Preserve modeled class/proxy copy. Do not export or display measured Mbps/Gbps truth. |
| F-10 | Switchable handover strategy | Phase 5 approves a route-local modeled replay policy preset selector. It is not live policy control. | `bounded-route-representation`; live policy truth remains forbidden/external | Preserve preset-only copy, telemetry, and reversibility. Do not imply operator policy control. |
| F-11 | Configurable / simulatable handover rules and related parameters | Phase 5 approves a route-local bounded replay rule/parameter preset surface. It is not an arbitrary rule editor. | `bounded-route-representation`; live rule-engine truth remains forbidden/external | Preserve preset-only copy, telemetry, and source-truth immutability. Do not alter projection fixtures or timelines. |
| F-12 | Handover decisions based on `latency/jitter/network speed` | Route represents modeled metric classes and handover pressure, not measured decision truth. | `bounded-route-representation` | Keep labels as modeled classes. |
| F-13 | Support at least `500 LEO` | Route is explicitly a 13-actor demo; current `>=500 LEO` evidence is accepted only through the separate fresh Phase 7.1 artifact `output/validation/phase7.1/2026-05-11T16-43-23.879Z-phase7-1-first-slice/summary.json`. | `external-validation-required` for this route; accepted via separate Phase 7.1 evidence boundary | Do not expand actor count or describe this route as scale validation. Keep the Phase 7.1 citation current. |
| F-14 | Adjustable simulation speed | Replay speed controls are already part of the viewer experience. | `true-route-closure` for route control | Regression guard only. |
| F-15 | Switch between `real time` and prerecorded `TLE` scenarios | Route is a bounded replay/demo route, not full multi-source scenario switching. | `bounded-route-representation` | Preserve replay disclosure. |
| F-16 | Export statistical reports | Route exposes a bounded JSON export action for route-owned demo state. It is not an external measurement/report-system export. | `bounded-route-representation`; external report-system truth remains `external-validation-required` | Preserve schema/version/provenance/non-claims and forbid external measurement/report truth claims. |
| F-17 | Show the impact of rain-attenuation scenarios | Repo seam exists mostly elsewhere; V4 route can show only gap/status or modeled context unless explicitly wired. | `bounded-repo-owned-seam` | Do not claim full physical truth. |
| F-18 | Run the display stably for at least `24 hours` | Phase 7.0 soak harness is landed; retained full-run artifact has not yet been produced — gate remains open pending a completed 24h run. | `soak-harness-ready; artifact-pending` | Do not claim route stability closed until the full-run artifact is retained. |
| V-01 | Linux as primary environment | Current development and validation are Linux/WSL-backed. | `true-route-closure` for dev baseline | Build and smoke on Linux. |
| V-02 | Windows + WSL as supported environment | Validation seam names WSL, but real tunnel setup is external. | `external-validation-required` | Plan evidence through `itri-v02-v06-external-validation-evidence-package-plan.md`; status/gap only in this route. |
| V-03 | tunneling / bridging validation scenario | Not mounted as route-owned network validation. | `external-validation-required` | Plan evidence through `itri-v02-v06-external-validation-evidence-package-plan.md`; status/gap only in this route. |
| V-04 | NAT routing connecting simulated network to a real network | Not mounted as route-owned NAT validation. | `external-validation-required` | Plan evidence through `itri-v02-v06-external-validation-evidence-package-plan.md`; status/gap only in this route. |
| V-05 | Virtual device-under-test scenario | Bounded validation state exists elsewhere; not route-owned DUT runtime. | `external-validation-required` | Plan evidence through `itri-v02-v06-external-validation-evidence-package-plan.md`; status/gap only in this route. |
| V-06 | Physical device-under-test / traffic generator scenario | Not route-owned. | `external-validation-required` | Plan evidence through `itri-v02-v06-external-validation-evidence-package-plan.md`; status/gap only in this route. |
| P-01 | Antenna parameters | Bounded physical-input seam exists elsewhere. | `bounded-repo-owned-seam` | Status/gap only unless explicitly wired. |
| P-02 | Rain fade / rain attenuation | Bounded physical-input seam exists elsewhere. | `bounded-repo-owned-seam` | Status/gap only unless explicitly wired. |
| P-03 | `ITU`-related factors | Bounded physical-input seam exists elsewhere. | `bounded-repo-owned-seam` | Status/gap only unless explicitly wired. |
| D-01 | Graphical / demonstration value, not purely backend | Route is a visual 3D handover review scene. | `true-route-closure` | Preserve visual quality and legibility. |
| D-02 | Supports real-time or prerecorded scenario demo | Route supports bounded replay/demo behavior. | `bounded-route-representation` | Preserve replay boundary. |
| D-03 | Visuals convey communication, handover, satellite, and link state | Link-flow cue improved ground-station/satellite relation expression. | `bounded-route-representation` | Guard link-flow cue telemetry and screenshots. |
