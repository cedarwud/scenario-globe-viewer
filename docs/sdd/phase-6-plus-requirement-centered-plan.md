...
| 500 LEO validation | `r1.docx`: `支援 ≥ 500 LEO 模擬`.<br>`kickoff.pptx` Slide 5: `支援 ≥ 500 LEO 模擬` | V4.12 closed the route-native LEO leg with `600` public Starlink TLE records; V4.13 carries that count inside the multi-orbit public-TLE gate | `7.1` | `multi-orbit-scale-1000` retained evidence shows `observedLeoCount = 600` with MEO/GEO counts also observed | Lower after V4.13: scale is evidenced for bounded public TLE, not for customer orbit-model or measured network truth |
