...
| 24h soak | `r1.docx` WP1 close-out and `kickoff.pptx` Slide 6: stable for at least `24` hours | Smoke/capture/hardening exist, but no formal soak procedure or retained soak evidence exists | `7.0` | Repeatable soak procedure, pass/fail rule, and retained failure evidence exist | Medium-high: short smoke may be mistaken for acceptance evidence |
