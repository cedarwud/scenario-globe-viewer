...
`scenario-globe-viewer` is an Earth-first Cesium delivery repo. The delivery
surface stays neutral, keeps Cesium upstream-owned, and preserves the
globe-first foundation even though the active repo now also includes accepted
replay, scenario, overlay, validation, and evidence surfaces through Phases 6.x
and 7.x.
...
2. Runtime shell
   `index.html`, `src/main.ts`, and app-shell modules own bootstrap sequencing, neutral page framing, `CESIUM_BASE_URL` setup, and top-level runtime composition.
3. Cesium integration seam
   `src/core/cesium/` modules wrap `Viewer`, bootstrap, provider-facing options, and scene-level controls without copying Cesium internals.
...
7. Phase 6 adds repo-owned scenario, operator, communication-time, handover-decision, physical-input, validation-state, and scene-starter surfaces on top of the preserved foundation. Those slices remain governed by the requirement-centered follow-on plan instead of retroactively redefining Phase 1-5.
8. Phase 7 adds retained soak and viewer-scope validation evidence boundaries. Those harnesses consume the accepted runtime/capture seams but do not become alternate runtime ownership surfaces.
9. Phase 8 local-view work remains downstream of the accepted Phase 6 and Phase 7 closure set. It should follow the dedicated Phase 8 SDD rather than widening this architecture file into a new product plan.
...
- Phase 6: requirement-centered scenario, operator, communication-time, handover-decision, physical-input, validation-state, and scene-starter surfaces
- Phase 7: soak and viewer-scope validation evidence
- Phase 8: downstream same-page local-view integration

Runtime implementation is no longer limited to Phase 5. Any follow-on change
beyond the landed Phase 7 evidence set should defer to the current Phase 6+,
Phase 8, and structural-refactor SDDs rather than treating this file as the
only planning surface.
