...
6. Phase 4 is still the fixture-ingestion boundary, and the landed Phase 4.1 slice remains limited to the walker smoke fixture adapter seam rather than widening the public contracts. If a future source starts from TLE / SGP4 / TEME / ECI output, parsing, propagation, and frame conversion into repo-facing ECEF sample data or other adapter-local Cesium-consumable forms remain adapter-owned rather than `overlay-manager`-owned. Phases 5.1-5.2 are the first live consumers of that seam, but they stay at a default-off walker point path with fixed labels only and still do not widen into orbits, overlay UI, or richer scenario data.
