...
6. Phase 4 is still the fixture-ingestion boundary, and the landed Phase 4.1 slice remains limited to the walker smoke fixture adapter seam rather than widening the public contracts. If a future source starts from TLE / SGP4 / TEME / ECI output, parsing, propagation, and frame conversion into repo-facing ECEF sample data or other adapter-local Cesium-consumable forms remain adapter-owned rather than `overlay-manager`-owned. Phase 5.1 is the first live consumer of that seam, but it stays at a default-off, point-only overlay path with no labels, orbits, or overlay UI.
