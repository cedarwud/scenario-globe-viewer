...
| Simulation disclosure | V4.10 hid important non-claim context behind a Truth button. | Conv 3 removes the Truth button and moves disclosure to footer `[Simulation view]`, with W5 `Not actual failover evidence`. | Q4 can be answered from the first viewport without opening Details. |
...
| Countdown/next state | V4.10 depended on sequence rail movement and copy. | Conv 2 adds a simulated countdown with `~ = simulated value`; Conv 3 footer retains disclosure. | Q3 has a stronger next-moment cue while preserving simulation boundaries. |
