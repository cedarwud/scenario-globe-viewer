...
const outputDir = path.join(repoRoot, "docs/images/phase-5.2-labels");
const outputFile = "global-preset-overlay-labels.png";
...
function findHeadlessBrowser() {
  ...
  throw new Error(
    "Missing a supported headless browser. Install google-chrome or chromium to capture the Phase 5.2 label review artifact."
  );
}
...
function startHeadlessBrowser(browserCommand) {
  const userDataDir = path.join(
    tmpdir(),
    `scenario-globe-viewer-phase5.2-labels-${process.pid}-${Date.now()}`
  );
...
async function waitForBootstrapReady(client) {
  ...
  for (let attempt = 0; attempt < 240; attempt += 1) {
    ...
    if (lastState.bootstrapState === "error") {
      throw new Error(
        `Phase 5.2 label capture hit bootstrap error: ${JSON.stringify(lastState)}`
      );
    }
  ...
  throw new Error(
    `Phase 5.2 label capture did not reach a ready viewer: ${JSON.stringify(lastState)}`
  );
}
...
async function enableLabelOverlay(client) {
  ...
  assert(
    result?.ok === true,
    `Phase 5.2 label capture failed to enable the overlay: ${JSON.stringify(result)}`
  );
}
        ...
        for (let index = 0; index < viewer.dataSources.length; index += 1) {
          ...
          for (const entity of dataSource.entities.values) {
            ...
            if (
              ...
              if (
                entity.label &&
                typeof entity.label.text?.getValue === "function" &&
                typeof entity.label.text.getValue(viewer.clock.currentTime) === "string"
              ) {
...
async function waitForOverlayReady(client) {
  ...
  for (let attempt = 0; attempt < 240; attempt += 1) {
    ...
    if (
      ...
      lastState.overlayRenderMode === "point-only" &&
      ...
      lastState.controllerState?.polylineCount === 0 &&
    ...
    if (lastState.overlayState === "error") {
      throw new Error(
        `Phase 5.2 label capture hit overlay error: ${JSON.stringify(lastState)}`
      );
    }
  ...
  throw new Error(
    `Phase 5.2 label capture did not reach ready overlay state: ${JSON.stringify(lastState)}`
  );
}
...
async function waitForViewerSettle(client) {
  ...
  for (let attempt = 0; attempt < 120; attempt += 1) {
    ...
    assert(nextState, "Missing capture viewer handle while settling Phase 5.2 label view.");
  ...
  throw new Error("Timed out waiting for settled Phase 5.2 label viewer state.");
}
...
async function captureScenario(baseUrl, browserCommand) {
  ...
  try {
    ...
    try {
      ...
      await enableLabelOverlay(client);
      ...
      console.log(
        `Captured Phase 5.2 label review artifact at ${path.relative(repoRoot, outputPath)}`
      );
    } finally {
      await client.close();
    }
...
async function main() {
  assertAmbientSiteTilesetUrlAllowed("phase5.2-label-capture");
