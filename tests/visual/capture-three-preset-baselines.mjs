...
const outputDir = path.join(repoRoot, "docs/images/phase-2.12");
...
function findHeadlessBrowser() {
  ...
  throw new Error(
    "Missing a supported headless browser. Install google-chrome or chromium to capture Phase 2.12 baselines."
  );
}
...
function startHeadlessBrowser(browserCommand) {
  const userDataDir = path.join(tmpdir(), `scenario-globe-viewer-visual-${process.pid}-${Date.now()}`);
  ...
  return await evaluateValue(
    ...
    `(() => {
      ...
      return {
        ...
        navigationHelpVisible: Boolean(
          document.querySelector('.cesium-click-navigation-help-visible')
        )
      };
    })()`
  );
...
async function waitForBootstrapReady(client, expectedScenePreset, scenarioKey) {
  let lastState = null;
  ...
  for (let attempt = 0; attempt < 240; attempt += 1) {
    ...
    if (lastState.bootstrapState === "error") {
      throw new Error(
        `Visual capture hit bootstrap error for ${scenarioKey}: ${JSON.stringify(lastState)}`
      );
    }
  ...
  throw new Error(
    `Visual capture did not reach a ready viewer for ${scenarioKey}: ${JSON.stringify(lastState)}`
  );
}
...
async function waitForViewerSettle(client, scenarioKey) {
  ...
  for (let attempt = 0; attempt < 120; attempt += 1) {
    ...
    assert(nextState, `Missing capture viewer handle while settling ${scenarioKey}.`);
  ...
  throw new Error(`Timed out waiting for settled viewer state for ${scenarioKey}.`);
}
...
async function captureScenario(baseUrl, browserCommand, scenario) {
  ...
  try {
    ...
    try {
      ...
      await waitForBootstrapReady(client, scenario.expectedScenePreset, scenario.key);
      ...
      console.log(`Captured ${scenario.key} baseline at ${path.relative(repoRoot, outputPath)}`);
    } finally {
      await client.close();
    }
  } finally {
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Phase 2.12 three-preset baselines captured under ${path.relative(repoRoot, outputDir)}`
    );
  } finally {
    await new Promise((resolve) => {
      server.once("exit", () => {
