...
const outputDir = path.join(repoRoot, "docs/images/formal-site-dataset-mvp");
const outputFile = "site-preset-dataset-enabled.png";
...
function findHeadlessBrowser() {
  ...
  throw new Error(
    "Missing a supported headless browser. Install google-chrome or chromium to capture the site dataset artifact."
  );
}
...
function startHeadlessBrowser(browserCommand) {
  const userDataDir = path.join(tmpdir(), `scenario-globe-viewer-site-dataset-${process.pid}-${Date.now()}`);
...
async function waitForBootstrapReady(client) {
  ...
  for (let attempt = 0; attempt < 240; attempt += 1) {
    ...
    if (
      ...
      lastState.scenePreset === "site" &&
      lastState.buildingShowcase === "off" &&
      lastState.buildingShowcaseSource === "default-off" &&
      lastState.buildingShowcaseState === "disabled" &&
      lastState.siteTilesetState === "ready" &&
      lastState.siteTilesetDetail?.includes("Loaded configured site dataset") &&
    ...
    if (lastState.bootstrapState === "error") {
      throw new Error(
        `Dataset capture hit bootstrap error: ${JSON.stringify(lastState)}`
      );
    }
  ...
  throw new Error(
    `Dataset capture did not reach a ready viewer: ${JSON.stringify(lastState)}`
  );
}
...
async function waitForViewerSettle(client) {
  ...
  for (let attempt = 0; attempt < 120; attempt += 1) {
    ...
    assert(nextState, "Missing capture viewer handle while settling site dataset view.");
  ...
  throw new Error("Timed out waiting for settled site dataset viewer state.");
}
...
async function captureScenario(baseUrl, browserCommand) {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Captured site dataset artifact at ${path.relative(repoRoot, outputPath)}`
      );
    } finally {
      await client.close();
    }
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Site dataset artifact captured under ${path.relative(repoRoot, outputDir)}`
    );
  } finally {
    await new Promise((resolve) => {
      server.once("exit", () => {
