...
function resolveExpectedReplayFacts() {
  ...
  assert(
    oneWebLeoActors.length === 3,
    `Expected the current V4.6A artifact to keep 3 OneWeb LEO actors, received ${oneWebLeoActors.length}.`
  );
...
async function main() {
  ...
  assert(
    expectedFacts.endpointIds.length === 2 &&
      expectedFacts.actorIds.length === 5 &&
      expectedFacts.orbitActorCounts.leo === 3 &&
      expectedFacts.orbitActorCounts.meo === 1 &&
      expectedFacts.orbitActorCounts.geo === 1,
    `V4.6A must not change the accepted artifact endpoint or actor set: ${JSON.stringify(
      expectedFacts
    )}`
  );
        ...
        const result = await evaluateRuntimeValue(
          ...
          `((config) => {
            ...
            assert(
              ...
              "V4.6A must not add or remove orbit actors: " +
                JSON.stringify({
                  actorIds,
