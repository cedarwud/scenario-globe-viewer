...
Phase 5.1 overlay toggle coverage is exposed through `npm run test:phase5.1`: it rebuilds `dist/`, serves the built app locally, proves the default globe-only shell still starts with the overlay disabled, toggles the repo-owned top-level satellite overlay controller on through the existing capture seam, verifies that walker-backed points attach through the new runtime path without adding labels/orbits/per-satellite controls, and then toggles that same path back off to confirm clean detachment while the native toolbar, timeline, credits, and status-only HUD remain intact.
