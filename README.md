...
- the current `3.6` overlay slice now adds a repo-owned overlay manager contract under `src/features/overlays/`; it keeps app-facing overlay state serializable, assigns attach/detach, top-level visibility, and disposal ownership to the manager boundary, reserves only a minimal future clock-binding hook for adapters, and still does not introduce a live overlay runtime, a satellite adapter implementation, or a walker fixture path
...
- repo-local smoke and targeted validation commands now exist at `npm test`, `npm run test:phase1`, `npm run test:phase3.3`, `npm run test:phase3.4`, `npm run test:phase3.5`, and `npm run test:phase3.6`
