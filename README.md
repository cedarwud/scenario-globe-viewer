...
- the current Phase 6.1 slice now includes a repo-owned `scenario` module under `src/features/scenario/`; it records scenario identity/source/lifecycle boundaries, resolves downstream plain-data inputs, emits a deterministic switch plan, and is still not wired into `src/main.ts` or any live runtime coordination path
