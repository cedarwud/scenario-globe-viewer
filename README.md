...
- the current Phase 6.1 contract-planning slice now includes a repo-owned `scenario` module under `src/features/scenario/`; it records scenario identity/source/lifecycle boundaries as plain-data only and is not yet wired into `src/main.ts` or any runtime coordination path
