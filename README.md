...
| `npm run test:phase3.3` | Build the repo, open the default runtime in a headless browser, and verify the Phase 3.3 real-time replay-clock seam drives Cesium's native `viewer.clock` for `getState`, `play`, `pause`, `setMultiplier`, `seek`, `onTick`, and the explicit `prerecorded` pending boundary |
...
- the current `3.2/3.3` time slices now pair a plain-data replay-clock contract under `src/features/time/` with a Cesium-backed real-time implementation in `src/features/time/cesium-replay-clock.ts`; the runtime keeps `viewer.clock` as the only time source, exposes the adapter only through the narrow capture seam for targeted validation, and still leaves `setMode('prerecorded')` as an explicit Phase 3.4 pending error instead of faking prerecorded behavior
...
- repo-local smoke and targeted validation commands now exist at `npm test`, `npm run test:phase1`, and `npm run test:phase3.3`
