...
| `npm run test:phase4.1` | Verify the walker fixture adapter keeps TLE parsing, SGP4 propagation, and frame conversion inside the adapter seam while staying off the live runtime path |
| `npm run test:phase5.1` | Build the repo, open the default runtime in a headless browser, and verify the default-off walker-backed overlay toggle path now keeps points plus fixed labels on the existing point path without widening into orbit/UI work |
...
| `npm run capture:phase5.2-labels` | Reject `VITE_CESIUM_SITE_TILESET_URL` resolved from shell env or repo-local `.env*`, build the repo, enable the bounded label-only overlay path on the global preset through the capture seam, and write the separate Phase 5.2 label review artifact |
...
- the current Phase 5.1-5.2 satellite overlay slice stays behind the single repo-owned top-level controller, keeps default startup globe-only, reuses the existing walker ingestion seam, renders the landed point path plus fixed runtime-local labels from existing `name` or fallback `id`, keeps path/polyline counts at `0`, and cleans up the runtime path completely on disable/retry without adding overlay HUD controls or per-satellite UI
- the bounded Phase 5.2 label-only widening now also has its own separate review artifact line under `docs/images/phase-5.2-labels/`; it is distinct from both the dormant Phase 2.12 preset baselines and the formal site dataset MVP artifact
...
- repo-local smoke and targeted validation commands now exist at `npm test`, `npm run test:phase1`, `npm run test:phase3.3`, `npm run test:phase3.4`, `npm run test:phase3.5`, `npm run test:phase3.6`, and `npm run test:phase3.7`
