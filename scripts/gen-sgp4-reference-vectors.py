...
def main() -> int:
    ...
    out = repo / "tests" / "golden" / "sgp4-reference-vectors.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(artifact, indent=2) + "\n")
    print(f"Wrote {len(samples)} reference samples -> {out}")

    # Typed in-app mirror so src/ can render the WS-G reference exhibit without
    # importing across the tests/ boundary. Kept byte-identical to the golden by
    # tests/unit/sgp4-reference-vectors-bundle-sync.test.mjs.
    ts_path = repo / "src" / "features" / "multi-station-selector" / "sgp4-reference-vectors.ts"
    mirror = {
        "reference": artifact["reference"],
        "toleranceKm": artifact["toleranceKm"],
        "samples": [
            {
                "label": s["label"], "orbitClass": s["orbitClass"], "noradId": s["noradId"],
                "line1": s["line1"], "line2": s["line2"], "timeUtc": s["timeUtc"],
                "positionEciKm": s["positionEciKm"], "velocityEciKmPerSec": s["velocityEciKmPerSec"],
            }
            for s in samples
        ],
    }
    ...
    ts_path.write_text(header + json.dumps(mirror, indent=2) + ";\n")
    print(f"Wrote in-app mirror -> {ts_path}")
