...
function usage() {
  return [
    "Usage: node scripts/review-itri-external-validation-manifest.mjs --package output/validation/external-v02-v06/<timestamp>-external-validation",
    "",
    "Options:",
    "  --package <path>    Required package directory. Must resolve under output/validation/external-v02-v06/.",
...
async function loadReviewerModule() {
  const tempDir = await mkdtemp(path.join(tmpdir(), "sgv-v02r1-reviewer-"));
  ...
  try {
    ...
    const compiled = ts.transpileModule(source, {
      ...
      fileName: "external-validation-manifest-reviewer.ts"
    }).outputText;
    const modulePath = path.join(tempDir, "external-validation-manifest-reviewer.mjs");
...
export async function reviewItriExternalValidationManifestFromPath({
  packageInput,
  manifestInput,
  ...
  try {
    ...
    if (!(await exists(retainedRoot))) {
      return reviewer.reviewMissingItriExternalValidationPackage({
        packagePath: relativePackagePath,
        manifestPath: relativeManifestPath
      });
    }
    ...
    if (!(await exists(absoluteManifestPath))) {
      return reviewer.reviewMissingItriExternalValidationManifest({
        packagePath: relativePackagePath,
        manifestPath: relativeManifestPath
      });
    }
    ...
    try {
      ...
      return reviewer.reviewMalformedItriExternalValidationManifest({
        packagePath: relativePackagePath,
        manifestPath: relativeManifestPath,
    ...
    const declaredRefs = reviewer.collectItriExternalValidationArtifactRefs(manifest);
    const artifactRefCheck = await resolveArtifactRefs(
      absolutePackagePath,
      declaredRefs
    );
    return reviewer.reviewItriExternalValidationManifest({
      manifest,
      packagePath: relativePackagePath,
      manifestPath: relativeManifestPath,
      artifactRefCheck
